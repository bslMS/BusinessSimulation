...
model BrokenTransition "The inflow to B is proportional to the outflow from A"
  import BusinessSimulation.Units.{Rate,Amount};
  ...
  parameter Rate rate(min = 0) = 0 "Constant rate of outflow for the stock A" annotation(Dialog(enable = hasConstantRate));
